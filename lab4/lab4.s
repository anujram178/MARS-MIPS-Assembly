...
innerLoop:
    bge $t4, $t2, innerLoopEnd

    mul $t5, $t3, $t2       # $t5 is i*number_of_columns
    add $t5, $t5, $t4       # $t5 is i* num_of_columns + j
    addi $t6, $t5, 0        # store offset into t6
    
    add $t5, $a0, $t5       	# base input address + offset	

    lbu $t7, 0($t5)          # store into t7 number from array
    
    bge $t7, $a3, setPixelToMax
    li $t7, 0x00		# t7 has value to be stored
    
    add $t8, $a1, $t6		# output array address where t7 will be stored
    
    sb $t7, 0($t8)		# store pixel into output array

    addiu $t4, $t4, 1       # increment inner-loop counter

    b innerLoop    # branch unconditionally back to beginning of the inner loop
    

innerLoopEnd:
    addiu $t3, $t3, 1       # increment outer-loop counter

    b outerLoop    # branch unconditionally back to beginning of the outer loop

matrixEnd:
j finalEnd

setPixelToMax:
li $t7, 0xff		# t7 has value to be stored
...
outer_loop:
    bge $t3, $a3, outer_loop_end

    li $t4, 0               # initialize inner-loop counter to 0

inner_loop:
    bge $t4, $a3, inner_loop_end

    mul $t5, $t3, $a3       # $t5 = num_of_columns * i
    add $t5, $t5, $t4       # $t5 = num_of_columns * i + j
    add $t5, $a1, $t5       # $t5 = base address + (num_of_columns * i + j)
    
    lw $t0, 0($a2)		# M00
    mul $t1, $t4, $t0		# t1 =  M00 * x
    
    lw $t0, 4($a2)		# M01
    mul $t6, $t3, $t0		# t6 = M01 * y
    
    add $t1, $t1, $t6		# M00 * x + M01 * y
    lw $t0, 8($a2)		# M02
    
    add $t8, $t1, $t0		# x0 = M00 * x + M01 * y + M02
    
    
    lw $t0, 12($a2)		# M10
    mul $t1, $t4, $t0		# t1 =  M10 * x
    
    lw $t0, 16($a2)		# M11
    mul $t6, $t3, $t0		# t6 = M11 * y
    
    add $t1, $t1, $t6		# M10 * x + M11 * y
    lw $t0, 20($a2)		# M12
    
    add $t9, $t1, $t0		# y0 = M00 * x + M01 * y + M02
    
    bge $t9, $a3, setOutputPixelToZero			# y0 >= total rows/ dimension
    bge $t8, $a3, setOutputPixelToZero			# x0 >= total columns/ dimension
    blt $t9, $zero, setOutputPixelToZero		# y0 < 0
    blt $t8, $zero, setOutputPixelToZero		# x0 < 0
    
    mul $t0, $t9, $a3       # $t0 =  dimension * y0
    add $t0, $t0, $t8       # $t0 = dimnesion * y0 + x0
    add $t0, $a0, $t0       # $t0 = base input address + (dimension * i + j)
    
    lbu $t1, 0($t0)		# load pixel from input array so that we can store into output array
    sb $t1, 0($t5)		# store pixel into output array

    addiu $t4, $t4, 1       # increment inner-loop counter

    b inner_loop    # branch unconditionally back to beginning of the inner loop
...
outer_loop_end:
j completelyFinalEnding

setOutputPixelToZero:
sb $zero, 0($t5)		# store pixel into output array

addiu $t4, $t4, 1       # increment inner-loop counter

b inner_loop    # branch unconditionally back to beginning of the inner loop

completelyFinalEnding:
